...
def get_vector_store(chunks):
    ...
    vector_store = FAISS.from_texts(chunks,embedding=embeddings) # use the embedding object on the splitted text of pdf docs
...
def get_conversation_chain():
    ...
    model = ChatGoogleGenerativeAI(model = "gemini-pro", client=genai, temperatue = 0.3) # create object of gemini-pro
...
def user_input(user_question):
    ...
    #new_db = Chroma.from_texts(texts, embeddings).as_retriever(search_kwargs={"k":5})
...
def main():
    ...
    st.header("Chat with Hello Cloud powered by Gemini 💁")
